...
void main() {
    ...
    float displacement = noise(position * 0.1 + vec3(0.0, 0.0, time * 0.002));
    displacement += 0.5 * noise(position * 0.2 + vec3(time * 0.001, 0.0, 0.0));
...
void main() {
    vec4 baseColor = vec4(baseColorUniform, 1.0);
    
    // create plasma effect using noise
    float noise1 = turbulence(vPosition * 0.015 + time * 0.0002);
    float noise2 = turbulence(vPosition * 0.03 + vec3(0.0, time * 0.0004, 0.0));
    
    // create sunspots
    float sunspots = smoothstep(0.4, 0.6, snoise(vPosition * 0.05 + vec3(time * 0.0002, 0.0, 0.0)));
...
void main() {
    ...
    float noiseValue = snoise(vNormal * 2.0 + vec3(0.0, time * 0.002, 0.0));
    noiseValue += 0.5 * snoise(vNormal * 4.0 + vec3(time * 0.004, 0.0, 0.0));
    
    // create animated solar flares
    float flares = pow(max(0.0, snoise(vNormal * 3.0 + vec3(time * 0.001, 0.0, 0.0))), 4.0);
    ...
    float alpha = fresnelFactor * (0.7 + 0.3 * sin(time * 0.01 + noiseValue * 5.0));
